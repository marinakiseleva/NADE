...
class OrderlessMoGNADE(NADE):
    ...
    def __init__(self, n_visible, n_hidden, n_layers, n_components, nonlinearity="RLU"):
        ...
        self.add_parameter(TensorParameter("V_mu", (n_visible, n_hidden,
                                                    n_components), theano=True), optimise=True, regularise=False)
        self.add_parameter(TensorParameter(
            "b_mu", (n_visible, n_components), theano=True), optimise=True, regularise=False)
        self.add_parameter(TensorParameter("V_sigma", (n_visible, n_hidden,
                                                       n_components), theano=True), optimise=True, regularise=False)
        self.add_parameter(TensorParameter(
            "b_sigma", (n_visible, n_components), theano=True), optimise=True, regularise=False)
        self.add_parameter(TensorParameter("V_alpha", (n_visible, n_hidden,
                                                       n_components), theano=True), optimise=True, regularise=False)
        self.add_parameter(TensorParameter(
            "b_alpha", (n_visible, n_components), theano=True), optimise=True, regularise=False)
        self.setup_n_orderings(1)
        self.recompile()

    @classmethod
    def create_from_params(cls, params):
        n_visible, n_hidden, n_layers, n_components = (
            params["n_visible"], params["n_hidden"], params["n_layers"], params["n_components"])
    ...
    def create_from_smaller_NADE(cls, small_NADE, add_n_hiddens=1, W_initialiser=Gaussian(std=0.01), domains=None):
        ...
        model = cls(n_visible, n_hidden, n_layers +
                    add_n_hiddens, n_components, nonlinearity)
        # Copy first layer
...
#         self.compiled_gradient_mask_logdensity = theano.function([x, m], g[par], allow_input_downcast=True)
    ...
    def logdensity(self, x):
        ...
        for o_index, o in enumerate(self.orderings):
            ...
            for j in xrange(self.n_visible):
                ...
                lp[:, o_index] += logsumexp(-0.5 * ((Mu - x[i][:, np.newaxis]) / Sigma)
                                            ** 2 - np.log(Sigma) - 0.5 * np.log(2 * np.pi) + np.log(Alpha))
    ...
    def sym_mask_logdensity_estimator_intermediate(self, x, mask):
        ...
        z_alpha = T.tensordot(
            h, self.V_alpha, [[1], [1]]) + T.shape_padleft(self.b_alpha)
        z_mu = T.tensordot(h, self.V_mu, [[1], [1]]) + T.shape_padleft(self.b_mu)
        z_sigma = T.tensordot(
            h, self.V_sigma, [[1], [1]]) + T.shape_padleft(self.b_sigma)
    ...
    def sym_mask_logdensity_estimator(self, x, mask):
        ...
        logdensity, z_alpha, z_mu, z_sigma, Alpha, Mu, Sigma, top_h = self.sym_mask_logdensity_estimator_intermediate(
            x, mask)
        return logdensity
...
#        lp_current_sum = (lp_current.sum(1) * D / (D-d)).sum() #1
        ...
        gV_alpha = T.tensordot(
            h.T, dp_dz_alpha, [[1], [0]]).dimshuffle((1, 0, 2))  # DxHxC
        ...
        gV_sigma = T.tensordot(
            h.T, dp_dz_sigma, [[1], [0]]).dimshuffle((1, 0, 2))  # DxHxC

        if self.n_layers > 1:
            gWs, gbs, gW1, gWflags, gb1 = T.grad(
                loss, [self.Ws, self.bs, self.W1, self.Wflags, self.b1])
            gradients = {"V_alpha": gV_alpha, "b_alpha": gb_alpha, "V_mu": gV_mu, "b_mu": gb_mu, "V_sigma": gV_sigma,
                         "b_sigma": gb_sigma, "Ws": gWs, "bs": gbs, "W1": gW1, "b1": gb1, "Wflags": gWflags}
        else:
            gW1, gWflags, gb1 = T.grad(loss, [self.W1, self.Wflags, self.b1])
            gradients = {"V_alpha": gV_alpha, "b_alpha": gb_alpha, "V_mu": gV_mu, "b_mu": gb_mu,
                         "V_sigma": gV_sigma, "b_sigma": gb_sigma, "W1": gW1, "b1": gb1, "Wflags": gWflags}
        # Gradients
        return (loss, gradients)
            ...
            for j in xrange(self.n_visible):
                ...
                if j < self.n_visible - 1:
                    a += np.dot(samples[i, s][np.newaxis, np.newaxis],
                                W1[i][np.newaxis, :]).flatten() + Wflags[i]
        return samples
    ...
    def conditional_logdensity(self, x):
        ...
        for o_index, o in enumerate(self.orderings):
            ...
            for j in xrange(self.n_visible):
                ...
                Mu = z_mu  # C
                Sigma = np.exp(z_sigma)
                conditionals[o_index, i] += logsumexp(-0.5 * ((Mu - x[i]) / Sigma) ** 2 - np.log(
                    Sigma) - 0.5 * np.log(2 * np.pi) + np.log(Alpha), axis=1)
                if j < self.n_visible - 1:
                    a += x[i] * W1[i] + Wflags[i]
    ...
    def conditional_logdensities(self, x, ys):
        ...
        for o_index, o in enumerate(self.orderings):
            ...
            for j in xrange(self.n_visible):
                ...
                Mu = z_mu  # C
                Sigma = np.exp(z_sigma)
                conditionals[o_index, i, :] += logsumexp(-0.5 * ((Mu[np.newaxis, :] - ys[
                                                         :, np.newaxis]) / Sigma) ** 2 - np.log(Sigma) - 0.5 * np.log(2 * np.pi) + np.log(Alpha), axis=1)
                a += x[i] * W1[i] + Wflags[i]
        return logsumexp(conditionals + np.log(1 / self.n_orderings), axis=0)
