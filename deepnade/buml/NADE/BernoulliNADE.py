...
class BernoulliNADE(NADE):
    ...
    def __init__(self, n_visible, n_hidden, nonlinearity="RLU"):
        ...
        self.add_parameter(TensorParameter("b", (n_visible), theano=True),
                           optimise=True, regularise=False)
        self.add_parameter(TensorParameter("activation_rescaling",
                                           (n_visible), theano=True), optimise=False, regularise=False)
        self.recompile()
    ...
    def initialize_parameters(self, marginal, W_initialiser=Gaussian(std=0.01)):
        ...
        self.activation_rescaling.set_value(
            np.ones(self.activation_rescaling.get_value().shape).astype(floatX))
    ...
    def sym_logdensity(self, x):
        ...
        def density_given_previous_a_and_x(x, w, v, b, activations_factor, p_prev, a_prev, x_prev):
            ...
            t = T.dot(h, v) + b
            # Make logistic regression more robust by having the sigmoid saturate at
            # 0.00005 and 0.99995
            p_xi_is_one = T.nnet.sigmoid(t) * constantX(0.9999) + constantX(0.0001 * 0.5)
        ...
        ([ps, _, _], updates) = theano.scan(density_given_previous_a_and_x,
                                            sequences=[x, self.W, self.V,
                                                       self.b, self.activation_rescaling],
                                            outputs_info=[p0, a0, x0])
        return (ps[-1], updates)


class BernoulliNADE_FixedBackground(NADE):
    # This version calculates the bias on the fly, by interpreting the
    # logistic units as linear energy model Bayes' classifiers, fixing one of
    # the models to assign p=0.5 to each input

    def __init__(self, n_visible, n_hidden, nonlinearity="sigmoid"):
        NADE.__init__(self, n_visible, n_hidden, nonlinearity)
        self.add_parameter(TensorParameter("W", (n_visible, n_hidden),
                                           theano=True), optimise=True, regularise=True)
        self.add_parameter(TensorParameter("b", (n_hidden), theano=True),
                           optimise=True, regularise=False)
        self.add_parameter(TensorParameter("V", (n_visible, n_hidden),
                                           theano=True), optimise=True, regularise=False)
...
class BernoulliNADE_FittedBackground(NADE):
    # This version calculates the bias on the fly, by interpreting the
    # logistic units as linear energy model Bayes' classifiers, the background
    ...
    def __init__(self, n_visible, n_hidden, nonlinearity="sigmoid"):
        ...
        self.add_parameter(TensorParameter("W", (n_visible, n_hidden),
                                           theano=True), optimise=True, regularise=True)
        self.add_parameter(TensorParameter("WB", (n_visible, n_hidden),
                                           theano=True), optimise=True, regularise=True)
        self.add_parameter(TensorParameter("b", (n_hidden), theano=True),
                           optimise=True, regularise=False)
        self.add_parameter(TensorParameter("V", (n_visible, n_hidden),
...
class BernoulliNADE_FittedBackgroundSparse(NADE):
    ...
    def __init__(self, n_visible, n_hidden, nonlinearity="sigmoid"):
        ...
        self.add_parameter(TensorParameter("WB", (n_visible, n_hidden),
                                           theano=True), optimise=True, regularise=True)
        self.add_parameter(TensorParameter("b", (n_hidden), theano=True),
                           optimise=False, regularise=False)
        self.add_parameter(TensorParameter("V", (n_visible, n_hidden),
                                           theano=True), optimise=True, regularise=False)
        self.add_parameter(TensorParameter("c", (n_visible), theano=True),
                           optimise=True, regularise=False)
