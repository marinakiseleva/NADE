...
class Sparse(ParameterInitialiser):
    ...
    def get_tensor(self, shape):
        def sparse_vector(l, n):
            ...
            vec[permutation[:n]] = np.random.normal(
                scale=self.std, size=(n,)).astype(np.float32)
            return vec
        if len(shape) == 1:
    ...
    def get_values(self, n_visible, n_hidden=1):
        # Sparse can give the number of non-zero connections or the proportion of
        # units from the visible layers to which each hidden unit connects
